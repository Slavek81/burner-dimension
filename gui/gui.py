...
class BurnerCalculatorGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        try:
            ...
            messagebox.showerror("Chyba inicializace", f"Chyba při načítání výpočetních modulů: {e}")
            return
    ...
    def create_input_tab(self):
        ...
        # Fuel flow rate
        ttk.Label(flow_frame, text="Hmotnostní průtok paliva [kg/s]:").grid(row=0, column=0, sticky="w")
        ...
        # Excess air ratio
        ttk.Label(flow_frame, text="Koeficient přebytku vzduchu [-]:").grid(row=1, column=0, sticky="w")
        ...
        ttk.Label(burner_frame, text="Maximální rychlost plynu [m/s]:").grid(row=0, column=0, sticky="w")
        ...
        ttk.Label(chamber_frame, text="Požadovaný tepelný výkon [kW]:").grid(row=0, column=0, sticky="w")
        ...
        ttk.Label(chamber_frame, text="Maximální teplota komory [°C]:").grid(row=1, column=0, sticky="w")
        ...
        ttk.Label(chamber_frame, text="Hustota tepelného toku [kW/m²]:").grid(row=2, column=0, sticky="w")
    ...
    def create_combustion_tab(self):
        ...
        # Results display
        self.combustion_text = scrolledtext.ScrolledText(combustion_frame, height=20, font=('Courier', 10))
    ...
    def create_chamber_tab(self):
        ...
        # Results display
        self.chamber_text = scrolledtext.ScrolledText(chamber_frame, height=20, font=('Courier', 10))
    ...
    def create_radiation_tab(self):
        ...
        # Results display
        self.radiation_text = scrolledtext.ScrolledText(radiation_frame, height=20, font=('Courier', 10))
    ...
    def create_pressure_tab(self):
        ...
        # Results display
        self.pressure_text = scrolledtext.ScrolledText(pressure_frame, height=20, font=('Courier', 10))
    ...
    def create_results_tab(self):
        ...
        # Results display
        self.results_text = scrolledtext.ScrolledText(results_frame, height=20, font=('Courier', 10))
    ...
    def create_control_buttons(self, parent):
        ...
        # Buttons
        ttk.Button(button_frame, text="Načíst vstup", command=self.load_input_file).pack(side="right", padx=(0, 5))
        ttk.Button(button_frame, text="Uložit vstup", command=self.save_input_file).pack(side="right", padx=(0, 5))
        ttk.Button(button_frame, text="Exportovat výsledky", command=self.export_results).pack(side="right", padx=(0, 5))
        ttk.Button(button_frame, text="Spustit výpočet", command=self.run_calculations, 
    ...
    def validate_input(self) -> bool:
        ...
        try:
            ...
            if self.input_data['ambient_temperature'] < -50 or self.input_data['ambient_temperature'] > 100:
                self.validation_errors.append("Teplota okolí musí být mezi -50°C a 100°C")
            
            if self.input_data['max_chamber_temp'] < 500 or self.input_data['max_chamber_temp'] > 2000:
                self.validation_errors.append("Maximální teplota komory musí být mezi 500°C a 2000°C")
            
            # Validate pressures
            if self.input_data['ambient_pressure'] < 50000 or self.input_data['ambient_pressure'] > 200000:
    ...
    def collect_input_data(self):
        ...
        numeric_fields = [
            'fuel_flow_rate', 'excess_air_ratio', 'ambient_temperature', 
            'ambient_pressure', 'max_gas_velocity', 'supply_pressure',
            'heat_output', 'max_chamber_temp', 'heat_release_density'
        ]
    ...
    def _display_summary_results(self):
        ...
        # Key results summary
        if all(key in self.results for key in ['combustion', 'burner', 'chamber', 'radiation', 'pressure']):
    ...
    def new_project(self):
        ...
        result = messagebox.askyesno("Nový projekt", 
        ...
        if result:
            ...
            for text_widget in [self.combustion_text, self.burner_text, self.chamber_text,
                               self.radiation_text, self.pressure_text, self.results_text]:
...
class ExportDialog:
    ...
    def create_export_widgets(self):
        ...
        ttk.Radiobutton(format_frame, text="TXT soubor", variable=self.format_var, value="txt").pack(anchor="w")
        ttk.Radiobutton(format_frame, text="CSV soubor", variable=self.format_var, value="csv").pack(anchor="w")
        ttk.Radiobutton(format_frame, text="Excel soubor", variable=self.format_var, value="excel").pack(anchor="w")
        ...
        ttk.Checkbutton(content_frame, text="Vstupní parametry", variable=self.include_input).pack(anchor="w")
        ttk.Checkbutton(content_frame, text="Výsledky výpočtů", variable=self.include_results).pack(anchor="w")
        ttk.Checkbutton(content_frame, text="Detailní údaje", variable=self.include_detailed).pack(anchor="w")
        
        # Buttons
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill="x", pady=(10, 0))
        
        ttk.Button(button_frame, text="Zrušit", command=self.dialog.destroy).pack(side="right", padx=(5, 0))
    ...
    def _export_excel(self, filename: str):
        ...
        try:
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                ...
                if self.include_input.get():
                    input_df = pd.DataFrame(list(self.input_data.items()), 
                ...
                if self.include_results.get():
                    for module_name, results in self.results.items():
                        if hasattr(results, '__dict__'):
                            result_df = pd.DataFrame(list(results.__dict__.items()),
                                                   columns=['Parametr', 'Hodnota'])
                            result_df.to_excel(writer, sheet_name=module_name.capitalize(), index=False)
                            
        except ImportError:
            # Fallback if pandas/openpyxl not available
            messagebox.showerror("Chyba", "Pro export do Excel je potřeba nainstalovat pandas a openpyxl")
