...
def main():
    ...
    try:
        ...
        fuel_flow_rate = required_power / combustion_calc.get_fuel_properties(fuel_type)['properties']['lower_heating_value_mass']
        ...
        print(f"Adiabatická teplota plamene: {combustion_results.adiabatic_flame_temperature:.0f} K ({combustion_results.adiabatic_flame_temperature-273.15:.0f} °C)")
        ...
        print(f"Hustota tepelného toku: {burner_results.heat_release_density/1e6:.2f} MW/m²")
        ...
        print(f"Celkový radiační přenos: {radiation_results.total_radiation_heat_transfer/1000:.1f} kW")
        print(f"Přenos plamen → stěna: {radiation_results.flame_to_wall_heat_transfer/1000:.1f} kW")
        print(f"Přenos stěna → okolí: {radiation_results.wall_to_ambient_heat_transfer/1000:.1f} kW")
        ...
        fittings = [
            Fitting('pipe_entrance_rounded', 1, pressure_calc.get_fitting_coefficient('pipe_entrance_rounded'), 0.05),
            ...
            Fitting('reducer_gradual', 1, pressure_calc.get_fitting_coefficient('reducer_gradual'), 0.025),
        ]
        ...
        print(f"Tlakový požadavek: {'✓' if pressure_ok else '✗'} {pressure_results.required_supply_pressure:.0f} Pa ≤ {supply_pressure:.0f} Pa")
        ...
        # Get recommendations
        burner_recommendations = burner_designer.get_design_recommendations(burner_results)
        chamber_recommendations = chamber_designer.get_design_recommendations(chamber_results)
        
        if burner_recommendations or chamber_recommendations:
            print("\nDOPOREČENÍ:")
            for i, rec in enumerate(burner_recommendations + chamber_recommendations, 1):
                print(f"{i}. {rec}")
