...
class BurnerReportGenerator:
    ...
    def set_metadata(self, 
                    ...
                    user_name: str = None,
                    software_version: str = "1.0.0") -> None:
        """
        ...
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                ...
                if 'combustion' in calculation_results:
                    ...
                    f.write(f"Teoretické množství vzduchu: {combustion.get('theoretical_air', 'N/A')} m³/m³\n")
                    f.write(f"Skutečné množství vzduchu: {combustion.get('actual_air', 'N/A')} m³/m³\n")
    ...
    def generate_csv_export(self, 
        ...
        try:
            ...
            with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                # Write metadata
                if self.report_metadata:
                    writer.writerow(['calculation_id', self.report_metadata.calculation_id, '-', 'metadata'])
                    writer.writerow(['timestamp', self.report_metadata.timestamp, '-', 'metadata'])
                    writer.writerow(['software_version', self.report_metadata.software_version, '-', 'metadata'])
        ...
        try:
            with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
                ...
                if 'burner' in calculation_results:
                    ...
                    summary_data['Hodnota'].extend([burner.get('power', 'N/A'), burner.get('nozzle_diameter', 'N/A')])
                    summary_data['Jednotka'].extend(['kW', 'mm'])
                
                if 'chamber' in calculation_results:
                    chamber = calculation_results['chamber']
                    summary_data['Parametr'].extend(['Objem komory', 'Doba zdržení'])
                    summary_data['Hodnota'].extend([chamber.get('volume', 'N/A'), chamber.get('residence_time', 'N/A')])
                ...
                if 'pressure_losses' in calculation_results and 'components' in calculation_results['pressure_losses']:
                    pressure_data = calculation_results['pressure_losses']['components']
                    pressure_df = pd.DataFrame(list(pressure_data.items()), 
                ...
                if self.report_metadata:
                    metadata_data = {
                        'Atribut': ['ID výpočtu', 'Datum a čas', 'Verze software', 'Projekt', 'Uživatel'],
                        'Hodnota': [
