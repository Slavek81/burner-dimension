...
class RadiationCalculator:
    ...
    def calculate_flame_radiation(
        ...
        max_theoretical_radiation = self.stefan_boltzmann * wall_area * (
            flame_temperature**4 - chamber_wall_temperature**4
        )
        
        radiation_efficiency = (total_radiation / max_theoretical_radiation) * 100 if max_theoretical_radiation > 0 else 0
    ...
    def _calculate_flame_emissivity(
        ...
        if soot_concentration > 0:
            soot_emissivity = 1 - math.exp(-self.SOOT_ABSORPTION_COEFF * soot_concentration * beam_length)
    ...
    def _calculate_flame_to_wall_radiation(
        ...
        emissivity_effective = 1 / (1/flame_emissivity + wall_area/(effective_flame_area * wall_emissivity) - 1)
    ...
    def calculate_radiation_exchange_network(
        self,
        surfaces: List[SurfaceProperties],
        view_factors_matrix: List[List[float]]
    ) -> Dict[str, float]:
        """
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if view_factor_ij > 0:
                    effective_emissivity = 1 / (1/emiss_i + (area_i/surfaces[j].area) * (1/emiss_j - 1))
