...
class BurnerDesigner:
    ...
    def __init__(self, combustion_calculator: CombustionCalculator = None, safety_factor: float = 1.2):
        """
    ...
    def validate_design(self, design_results: BurnerDesignResults) -> Dict[str, bool]:
        ...
        validation = {
            ...
            'reasonable_dimensions': (
                0.001 <= design_results.burner_diameter <= 1.0 and
                0.01 <= design_results.burner_length <= 5.0
            ),
    ...
    def get_design_recommendations(self, design_results: BurnerDesignResults) -> list:
        ...
        if not validation['velocity_in_range']:
            if design_results.gas_velocity < self.MIN_GAS_VELOCITY:
                recommendations.append("Rychlost plynu je příliš nízká - zvyšte tlak nebo zmenšete průměr")
            else:
                recommendations.append("Rychlost plynu je příliš vysoká - snižte tlak nebo zvětšete průměr")
        
        if not validation['heat_density_acceptable']:
            recommendations.append("Hustota tepelného toku je příliš vysoká - zvětšete plochu hořáku")
        
        if design_results.heat_release_density > 3e6:
            recommendations.append("Doporučuje se přídavné chlazení při vysoké hustotě tepelného toku")
