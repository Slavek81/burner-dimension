...
class ChamberDesigner:
    ...
    def calculate_temperature_distribution(
        ...
        for i in range(num_points + 1):
            ...
            temperature = (
                inlet_temperature * math.exp(-decay_constant * position) +
    ...
    def validate_design(self, design_results: ChamberDesignResults) -> Dict[str, bool]:
        ...
        validation = {
            ...
            'dimensions_reasonable': (
                self.MIN_CHAMBER_DIAMETER <= design_results.chamber_diameter <= self.MAX_CHAMBER_DIAMETER
            ),
    ...
    def get_design_recommendations(self, design_results: ChamberDesignResults) -> list:
        ...
        if not validation['volume_heat_rate_acceptable']:
            recommendations.append("Objemová hustota tepelného toku je příliš vysoká - zvětšete komoru")
        
        if not validation['efficiency_acceptable']:
            recommendations.append("Nízká tepelná účinnost - zvyšte izolaci nebo optimalizujte geometrii")
...
def main():
    ...
    try:
        ...
        temp_dist = designer.calculate_temperature_distribution(
            results, 
            designer.combustion_calc.calculate_combustion_products(fuel_type, 0.002, 1.2)
        )
