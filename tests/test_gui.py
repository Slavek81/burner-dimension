...
class TestBurnerCalculatorGUI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_calculators['combustion'].get_available_fuels.return_value = [
            'natural_gas', 'propane', 'methane'
        ]

    @patch('gui.CombustionCalculator')
    @patch('gui.BurnerDesigner')
    @patch('gui.ChamberDesigner')
    @patch('gui.RadiationCalculator')
    @patch('gui.PressureLossCalculator')
    @patch('gui.ttk')
    @patch('gui.tk')
    def test_initialization_success(self, mock_tk, mock_ttk, mock_pressure, 
                                   mock_radiation, mock_chamber, mock_burner, mock_combustion):
    ...
    @patch('gui.ttk')
    @patch('gui.tk')
    def test_initialization_calculator_error(self, mock_tk, mock_ttk, mock_combustion, mock_messagebox):
    ...
    def test_perform_calculations_success(self):
        ...
        with patch.object(BurnerCalculatorGUI, '__init__', lambda x, y: None):
            ...
            gui.calculators['combustion'].calculate_combustion_products.return_value = mock_combustion_result
            ...
            gui.calculators['radiation'].calculate_flame_radiation.return_value = mock_radiation_result
            
            # Call method
            gui._perform_calculations()
            
            # Verify calculations were called
            gui.calculators['combustion'].calculate_combustion_products.assert_called_once()
